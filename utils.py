...
class DKT(object):
    def __init__(self, sess, data_train, data_test, num_problems, network_config, save_dir_prefix='./',
        ...
        self.log_file_path = os.path.join(self.ckpt_save_dir, "{}_{}.log".format(model_name, str(time.time())))
    ...
    def evaluate(self, is_train=True):
        if is_train:
            data = self.data_train
        else:
            data = self.data_test
        ...
        for run_idx in range(num_runs):
            ...
            for epoch_idx in range(num_epochs):
                ...
                if auc_test > best_test_auc:
                    ...
                    best_waviness_l1, best_waviness_l2 = self.waviness(is_train=True)

                    # finding m1, m2
                    m1, m2 = self.consistency(is_train=True)
    ...
    def waviness(self, is_train=True):
        if is_train:
            data = self.data_train
        else:
            data = self.data_test
    ...
    def waviness_np(self, is_train=True):
        if is_train:
            data = self.data_train
        else:
            data = self.data_test
    ...
    def _reconstruction_accurarcy(self, is_train=True):
        if is_train:
            data = self.data_train
        else:
            data = self.data_test
    ...
    def consistency(self, is_train=True):
        if is_train:
            data = self.data_train
        else:
            data = self.data_test
